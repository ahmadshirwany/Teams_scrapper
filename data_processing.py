...
def calculate_average(a=None, b=None, c=None, d=None, e=None):
    ...
    # Check if there are valid values to avoid division by zero
    if len(valid_values) == 0:
        return 0  # or return None or a message
...
df_barttorvik = pd.read_csv('barttorvik_table.csv')[['Team', 'Barthag']]
df_barttorvik['Barthag'] = df_barttorvik['Barthag'] / df_barttorvik['Barthag'].iloc[0]
df_haslametrics = pd.read_csv('haslametrics_table.csv')[['Team', 'AP%']]
df_kenpom = pd.read_csv('kenpom_table.csv')[['Team', 'NetRtg']]
max_value = df_kenpom['NetRtg'].max()
df_kenpom['NetRtg'] = 100 - (max_value - df_kenpom['NetRtg'])
df_evanmiya = pd.read_csv('evanmiya_table.csv')[['Team', 'Relative Rating']]
...
for team_name_dict in team_names:
    ...
    if 'teamrankings' in team_name_dict:
        teamrankings_b = df_teamrankings[df_teamrankings['Team'] == team_name_dict['teamrankings']].values.tolist()[0][1]
    else:
        teamrankings_b = ''
